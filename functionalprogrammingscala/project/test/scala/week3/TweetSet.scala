...
class Tweet(val user: String, val text: String, val retweets: Int) {
  ...
  def mentions(words: List[String]): Boolean = words.exists(x => text.contains(x))
}
...
abstract class TweetSet extends TweetSetInterface:
  ...
  def descendingByRetweet: TweetList = 
    def loop(s: TweetSet, orderedList: TweetList): TweetList =
      if (s.isEmpty) orderedList
      else {
        val moreRetweets = s.mostRetweeted
        new Cons(moreRetweets, loop(s.remove(moreRetweets), orderedList))
      }
    loop(this, Nil)

  def mentions(words: List[String]): TweetSet = filter(x => x.mentions(words))
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  def mostRetweeted: Tweet = {
    ...
    val moreRetweets = all.filter(p => p.retweets > elem.retweets)
    if moreRetweets.isEmpty then elem else moreRetweets.mostRetweeted
  }
