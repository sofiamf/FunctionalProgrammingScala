...
object Anagrams extends AnagramsInterface:
   ...
   *
   */
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary.groupBy(a => wordOccurrences(a))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x::xs if x._2 >= 1 => {
      for {
        numOccurrences <- 0 to x._2
        comb <- combinations(xs)
      }
      yield ((x._1, numOccurrences) :: comb).filter(_._2 > 0)
    }.toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    val mapOfY = y.foldLeft(Map[Char, Int]())((acc, i) => acc + (i._1 -> i._2))
    x.map(occ => (occ._1, occ._2 - mapOfY.getOrElse(occ._1, 0))).filter(_._2 > 0)
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def aux(y: Occurrences): List[Sentence] = {
      if (y.isEmpty) List(List())
      else
        for (s <- combinations(y) filter dictionaryByOccurrences.contains;
